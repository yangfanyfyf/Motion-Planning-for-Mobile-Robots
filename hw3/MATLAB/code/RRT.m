...
for iter = 1:3000
    ...
    x_rand(1) = rand * size(Imp,1);
    x_rand(2) = rand * size(Imp,2);
    
    x_near=[];
    %Step 2: 遍历树，从树中找到最近邻近点x_near 
    %提示：x_near已经在树T里
    num = 0;
    minLength = Distance(x_rand, [T.v(1).x T.v(1).y]);
    for j = 1 : size(T.v,2)
        tempL = Distance(x_rand, [T.v(j).x T.v(j).y]);
        if tempL <= minLength
            num = j;
            minLength = tempL;
        end
    end
    x_near = [T.v(num).x T.v(num).y];
    ...
    %Step 3: 扩展得到x_new节点
    %提示：注意使用扩展步长Delta
    perL = Delta / minLength;
    x_new(1) = x_near(1) + perL * (x_rand(1) - x_near(1));
    x_new(2) = x_near(2) + perL * (x_rand(2) - x_near(2));
    ...
    T.v(count).indPrev = num;
    
    
    
    %Step 5:检查是否到达目标点附近 
    %提示：注意使用目标点阈值Thr，若当前节点和终点的欧式距离小于Thr，则跳出当前for循环
    if Distance(x_new, [x_G y_G]) < Thr
        break
    end
