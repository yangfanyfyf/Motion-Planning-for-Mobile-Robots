        ...
        for (int j = -1; j != 2; ++j){
            for (int k = -1; k != 2; ++k){
                ...
                if( n_x < 0 || (n_x > (GLX_SIZE - 1)) || n_y < 0 || (n_y > (GLY_SIZE - 1)) || n_z < 0 || (n_z > (GLZ_SIZE -1)) ){
                    continue; // not outside the map
                }

                if (isOccupied(n_x, n_y, n_z)){
                    continue; // not inside the Obstacles
                }
                ...
                if( (abs(n_coord[0] - this_coord[0]) < 1e-6) and (abs(n_coord[1] - this_coord[1]) < 1e-6) and (abs(n_coord[2] - this_coord[2]) < 1e-6 )){
                    cout << "Error: Not expanding correctly!" << endl;
                    cout << "n_coord:" << n_coord[0] << " "<<n_coord[1]<<" "<<n_coord[2] << endl;
                    cout << "this_coord:" << this_coord[0] << " "<<this_coord[1]<<" "<<this_coord[2] << endl;

                    cout << "current node index:" << this_x << " "<< this_y<<" "<< this_z << endl;
                    cout << "neighbor node index:" << n_x << " "<< n_y<<" "<< n_z << endl;
                }
    ...
    */
    ...
    while ( !openSet.empty() ){
        ...
        */
        ...
        GridNodeMap[current_idx[0]][current_idx[1]][current_idx[2]]->id = -1; // -1 == close set
